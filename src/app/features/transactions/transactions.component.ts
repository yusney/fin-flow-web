...
@Component({
  ...
  template: `
      ...
      <div class="p-4 lg:p-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div class="flex gap-3 lg:gap-4">
            <div class="bg-surface-container-lowest px-4 py-3 rounded-[var(--radius-card)] shadow-[var(--shadow-card)]">
              <div class="text-xs text-on-surface-variant uppercase tracking-wider">{{ t('transactions.income') }}</div>
              <div class="text-lg font-bold text-secondary tabular-nums">
                +{{ stats().totalIncome | currency: 'USD' : 'symbol' : '1.0-0' }}
              </div>
            </div>
            <div class="bg-surface-container-lowest px-4 py-3 rounded-[var(--radius-card)] shadow-[var(--shadow-card)]">
              <div class="text-xs text-on-surface-variant uppercase tracking-wider">{{ t('transactions.expenses') }}</div>
              <div class="text-lg font-bold text-tertiary tabular-nums">
                -{{ stats().totalExpenses | currency: 'USD' : 'symbol' : '1.0-0' }}
              </div>
            </div>
            <div class="bg-surface-container-lowest px-4 py-3 rounded-[var(--radius-card)] shadow-[var(--shadow-card)] hidden sm:block">
              <div class="text-xs text-on-surface-variant uppercase tracking-wider">{{ t('transactions.total') }}</div>
              <div class="text-lg font-bold text-on-surface tabular-nums">{{ stats().count }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="p-4 lg:p-8 pb-0">
      <div class="bg-surface-container-low rounded-[var(--radius-card)] p-4 lg:p-6 shadow-[var(--shadow-card)]">
        ...
        <div class="flex flex-col sm:flex-row gap-4 mt-4 pt-4 border-t border-outline-variant">
          ...
          <span class="text-on-surface-variant self-center">{{ t('transactions.dateSeparator') }}</span>
          <input type="date" [(ngModel)]="dateTo" (ngModelChange)="onDateToChange($event)"
            class="px-3 py-2 bg-surface-container-lowest rounded-[var(--radius-input)] text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/20" />
          @if (hasActiveFilters()) {
            <button (click)="clearFilters()" class="ml-auto flex items-center gap-1 text-sm text-primary hover:underline">
    ...
    <div class="p-4 lg:p-8">
      ...
      @if (!isLoading() && filteredTransactions().length === 0) {
        <div class="bg-surface-container-low rounded-[var(--radius-card)] p-12 text-center">
          <span class="material-symbols-outlined text-[48px] text-outline mb-4">receipt_long</span>
          <h3 class="text-lg font-bold font-headline text-on-surface mb-2">{{ t('transactions.emptyTitle') }}</h3>
          <p class="text-sm text-on-surface-variant mb-4">{{ t('transactions.emptySubtitle') }}</p>
      ...
      <div class="space-y-3">
        @for (transaction of filteredTransactions(); track transaction.id; let i = $index) {
          ...
          >
            <div class="flex items-center gap-x-3 lg:gap-x-4 min-w-0 flex-1" (click)="openEditModal(transaction)">
              <div [class]="iconBgClass(transaction)" class="w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center shrink-0">
                <span class="material-symbols-outlined text-[18px] lg:text-[24px]">{{ transaction.icon ?? 'payments' }}</span>
              </div>
              <div class="min-w-0 flex-1">
                <div class="font-bold font-headline text-on-surface text-sm lg:text-base truncate">{{ transaction.description }}</div>
                <div class="flex items-center gap-2 text-xs text-on-surface-variant">
                  <span>{{ transaction.date | date: 'MMM dd, yyyy' }}</span>
            ...
            <div class="flex items-center gap-x-4 shrink-0">
              ...
              <div class="flex items-center gap-1">
                ...
                <button (click)="confirmDelete(transaction); $event.stopPropagation()"
    ...
    @if (isModalOpen()) {
      <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="closeModal()">
        <div class="bg-surface-container-lowest rounded-[var(--radius-card)] w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-[var(--shadow-elevated)]" (click)="$event.stopPropagation()">
          <div class="flex items-center justify-between p-6 border-b border-outline-variant">
            <h2 class="text-xl font-bold font-headline text-on-surface">
              {{ modalMode() === 'create' ? t('transactions.modalTitleCreate') : t('transactions.modalTitleEdit') }}
            </h2>
            <button (click)="closeModal()" class="p-2 text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high rounded-full transition-colors">
          ...
          <form (ngSubmit)="saveTransaction()" class="p-6 space-y-4">
            <div>
              <label class="block text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-2">{{ t('transactions.fieldDescription') }}</label>
            ...
            <div>
              <label class="block text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-2">{{ t('transactions.fieldAmount') }}</label>
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant">$</span>
                <input type="number" [(ngModel)]="formData.amount" name="amount" required min="0.01" step="0.01" placeholder="0.00"
            ...
            <div>
              <label class="block text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-2">{{ t('transactions.fieldType') }}</label>
              <div class="flex gap-3">
                <label class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-[var(--radius-input)] cursor-pointer transition-all border-2"
                  ...
                  <input type="radio" [(ngModel)]="formData.type" name="type" value="income" class="hidden" (ngModelChange)="onFormTypeChange()" />
                  <span class="material-symbols-outlined text-secondary">trending_up</span>
                  <span class="font-medium" [class.text-secondary]="formData.type === 'income'">{{ t('transactions.typeIncome') }}</span>
                </label>
                <label class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-[var(--radius-input)] cursor-pointer transition-all border-2"
                  ...
                  <input type="radio" [(ngModel)]="formData.type" name="type" value="expense" class="hidden" (ngModelChange)="onFormTypeChange()" />
                  <span class="material-symbols-outlined text-tertiary">trending_down</span>
                  <span class="font-medium" [class.text-tertiary]="formData.type === 'expense'">{{ t('transactions.typeExpense') }}</span>
                </label>
              </div>
            </div>

            <div>
              <label class="block text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-2">{{ t('transactions.fieldCategory') }}</label>
            ...
            <div>
              <label class="block text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-2">{{ t('transactions.fieldDate') }}</label>
    ...
    @if (deleteModalOpen()) {
      <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="closeDeleteModal()">
        <div class="bg-surface-container-lowest rounded-[var(--radius-card)] w-full max-w-sm p-6 shadow-[var(--shadow-elevated)]" (click)="$event.stopPropagation()">
          <div class="text-center">
            <div class="w-12 h-12 bg-tertiary-container/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <span class="material-symbols-outlined text-tertiary text-[24px]">delete_forever</span>
            </div>
            <h3 class="text-lg font-bold font-headline text-on-surface mb-2">{{ t('transactions.deleteTitle') }}</h3>
    ...
    <!-- Toast -->
    @if (toast()) {
      <div class="fixed bottom-4 left-1/2 -translate-x-1/2 lg:bottom-8 z-50 flex items-center gap-2 px-4 py-3 rounded-full shadow-[var(--shadow-elevated)] animate-fade-in"
...
export class TransactionsComponent {
  ...
  onTypeChange(value: string): void { this.selectedType.set(value as 'income' | 'expense' | 'all'); }
  onCategoryChange(value: string): void { this.selectedCategoryId.set(value); }
  onDateFromChange(value: string): void { this.dateFrom.set(value); }
  onDateToChange(value: string): void { this.dateTo.set(value); }
  ...
  validateForm(): boolean {
    ...
    if (!this.formData.description.trim()) { this.formError.set(t('transactions.errorDescription')); return false; }
    if (this.formData.amount <= 0) { this.formError.set(t('transactions.errorAmount')); return false; }
    if (!this.formData.categoryId) { this.formError.set(t('transactions.errorCategory')); return false; }
    if (!this.formData.date) { this.formError.set(t('transactions.errorDate')); return false; }
    return true;
  }
  ...
  saveTransaction(): void {
    ...
    if (this.modalMode() === 'create') {
      this.transactionService.createTransaction(request).subscribe({
        ...
        error: (error) => {
          this.isSaving.set(false);
          this.formError.set(error.error?.message || this.transloco.translate('transactions.toastCreateError'));
        },
      });
    } else {
      ...
      if (transaction) {
        this.transactionService.updateTransaction({ ...request, id: transaction.id }).subscribe({
          ...
          error: (error) => {
            this.isSaving.set(false);
            this.formError.set(error.error?.message || this.transloco.translate('transactions.toastUpdateError'));
          },
        });
      }
    }
  ...
  executeDelete(): void {
    ...
    this.transactionService.deleteTransaction(transaction.id).subscribe({
      ...
      error: (error) => {
        this.isDeleting.set(false);
        this.showToast(error.error?.message || this.transloco.translate('transactions.toastDeleteError'), 'error');
      },
    });
  }
  ...
  iconBgClass(transaction: Transaction): string {
    ...
    if (transaction.category?.toLowerCase().includes('entertain')) return 'bg-primary-container/20 text-primary';
