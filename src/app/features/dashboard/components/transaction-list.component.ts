...
@Component({
  ...
  template: `
    ...
    <div class="flex justify-between items-end mb-4 lg:mb-6">
      ...
      <a routerLink="/transactions" class="text-xs lg:text-sm font-semibold text-primary hover:underline">
          ...
          <div class="flex items-center gap-x-3 lg:gap-x-4 min-w-0 flex-1">
            ...
            <div class="min-w-0 flex-1">
              ...
              <div class="text-xs text-on-surface-variant truncate">
                <span class="hidden sm:inline">{{ tx.date | date: 'MMMM dd, yyyy' }} • </span>
                <span class="sm:hidden">{{ tx.date | date: 'MMM dd' }} • </span>
                {{ tx.category }}
              </div>
            </div>
          </div>
